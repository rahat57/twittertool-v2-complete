...
public class ElasticsearchApi {
	...
	public  boolean deleteMaping(TransportClient client,String index,String type) throws Exception {
		...
		System.err.println(mappings.keys());
		BoolQueryBuilder boolQuery = new BoolQueryBuilder();
		boolQuery.should(QueryBuilders.matchPhraseQuery("date", type));
		if (mappings.containsKey(type)) { 
			DeleteByQueryResponse response1 = client.prepareDeleteByQuery().setIndices(index).setTypes(type)
			        .setQuery(boolQuery)
			        .execute()
			        .actionGet();
